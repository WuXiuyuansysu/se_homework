...
def generate_nutrition_analysis(recipe):
    ...
    参数:
        recipe (dict): 包含菜谱信息的字典，需包含以下字段：
            - 'name': 菜谱名称（字符串）
            - 'ingredients': 食材列表，每个元素是包含 'name' 和 'quantity' 的字典
            - 'steps': 制作步骤列表，每个元素为包含 'description' 的字典

    功能:
        - 构造专业提示词，基于菜谱名称、食材、关键步骤等信息生成营养分析；
        - 使用大语言模型分析热量、蛋白质、脂肪、碳水化合物、纤维及关键营养素；
        - 返回包含所有营养成分的 JSON 格式数据。
    ...
    返回:
        dict: 包含以下字段的营养分析数据：
            ...
            - 'dietary_fiber'（可选）: 膳食纤维含量（克）
            - 'key_nutrients': 一个数组，列出2-4种关键营养素，每个元素为字典，包含：
                - 'nutrient': 营养素名称（如“维生素C”）
                - 'amount': 含量（数值）
                - 'unit': 单位（如毫克、微克）
                - 'source': 主要来源食材

    注意:
        - 输出结果完全依赖于 AI 模型生成，需自行判断合理性；
        - 推荐添加异常处理以防网络或格式错误导致程序崩溃。
    """
